...
- 读取配置时会显示并保留自定义字体路径；安装时只有主动选择其他字体才会替换它。相对路径以游戏的 `game` 目录为基准。
- 跳过规则限定为管理器与游戏共同支持的基础正则语法：文字、字符类、常用转义、锚点、分支、普通组、非捕获组和次数限定。不支持命名组、反向引用、前后查找、内联选项及扩展转义；例如可用 `^(?:Hello|World)\s+\d{2,}$`。
- 关闭翻译后，确认框保留原文，尚未发送的批次不会发出；已发送的请求允许完成。重新开启后，未翻译文本可再次排队。

## 安装、卸载与数据

安装前校验译文格式及重复原文，写入前备份被覆盖文件，失败后恢复原始字节。安装目录中的配置、运行时缓存和用户其他文件不会被软件自更新覆盖。

安装记录损坏或缺少必要校验项时，会显示可修复状态，仍可读取有效配置并直接安装 / 修复。记录中有效的资源模式会保留；无法识别模式时默认选择通用模式，请在修复前确认。配置文件自身损坏、文件权限或链接错误仍需先处理。

默认卸载只删除通用与专属模组 `.rpy`、`.rpyc` 和桌面安装记录，保留配置、缓存、译文及字体。勾选“卸载时删除 game/live_translator 内全部文件”后，会递归删除该目录内的全部文件，包括配置、缓存、译文、字体和自行存放的文件，操作前仍会备份。原游戏文件、存档和其他目录不属于清理范围。
...
软件更新目录在管理器启动及准备更新时清理：保留最近 2 份，额外跳过创建不足一天或正在使用的目录；其余旧目录连同下载包、解包文件和 `previous` 备份一并回收。清理失败会留到下次重试。
...
默认版本来源是 `packaging/version.txt`，开发构建也读取该文件。发布脚本通过 `-p:Version` 注入程序集，并同步发行暂存目录的资源版本；参数或 `v*` 标签覆盖版本时，不改仓库内的默认版本文件。推送 `v*` 标签会自动构建、测试并发布 Release。本地提交不会自动发布，需另行推送标签。
